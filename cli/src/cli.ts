...
export class CLI {
	...
	async initialize(): Promise<void> {
		...
		try {
			logs.info("Initializing CoreThink Code CLI...", "CLI")
			...
			process.stdout.write(`\x1b]0;CoreThink Code - ${folderName}\x07`)
			...
			logs.info("CoreThink Code CLI initialized successfully", "CLI")
		} catch (error) {
	...
	async dispose(signal?: string): Promise<void> {
		...
		try {
			logs.info("Disposing CoreThink Code CLI...", "CLI")
			...
			logs.info("CoreThink Code CLI disposed", "CLI")
		} catch (error) {
