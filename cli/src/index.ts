...
program
	.name("kilocode")
	.description("CoreThink Code Terminal User Interface - AI-powered coding assistant")
	...
	.action(async (prompt, options) => {
		...
		if (!hasConfig && !hasEnvConfig) {
			...
			console.info("Welcome to the CoreThink Code CLI! 🎉\n")
		...
		logs.debug("Starting CoreThink Code CLI", "Index", { options })
...
program
	.command("auth")
	.description("Manage authentication for the CoreThink Code CLI")
	.action(async () => {
		await authWizard()
	})
...
program
	.command("debug")
	.description("Run a system compatibility check for the CoreThink Code CLI")
