---
title: Using MCP in CoreThink Code
sidebar_label: Using MCP in CoreThink Code
---

# Using MCP in CoreThink Code

Model Context Protocol (MCP) extends CoreThink Code's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with CoreThink Code.

<YouTubeEmbed
  url="https://youtu.be/6O9RQoQRX8A"
  caption="Demostrating MCP installation in CoreThink Code"
/>
...
2.  **Project-level Configuration**: Defined in a `.kilocode/mcp.json` file within your project's root directory. This allows you to set up project-specific servers and share configurations with your team by committing the file to version control. CoreThink Code automatically detects and loads this file if it exists.
...
You can edit both global and project-level MCP configuration files directly from the CoreThink Code MCP settings view.

1. Click the <Codicon name="gear" /> icon in the top navigation of the CoreThink Code pane to open `Settings`.
2. Click the `MCP Servers` tab on the left side
3. Choose the `Installed` servers
4. Click the appropriate button:
    - **`Edit Global MCP`**: Opens the global `mcp_settings.json` file.
    - **`Edit Project MCP`**: Opens the project-specific `.kilocode/mcp.json` file. If this file doesn't exist, CoreThink Code will create it for you.
...
_Example of MCP Server config in CoreThink Code (STDIO Transport)_
...
When enabled, CoreThink Code will automatically approve this specific tool without prompting. Note that the global "Use MCP servers" setting takes precedence - if it's disabled, no MCP tools will be auto-approved.

## Finding and Installing MCP Servers

CoreThink Code does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask CoreThink Code:** You can ask CoreThink Code to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend CoreThink Code with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, CoreThink Code will automatically detect available tools and resources. To use them:

1. Type your request in the CoreThink Code chat interface
2. CoreThink Code will identify when an MCP tool can help with your task
