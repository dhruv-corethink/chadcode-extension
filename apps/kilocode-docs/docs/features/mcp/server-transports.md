...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between CoreThink Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (CoreThink Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as CoreThink Code
...
- Single-client scenarios (one CoreThink Code instance per server)
...
1. The client (CoreThink Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from CoreThink Code
...
STDIO servers run locally on the same machine as CoreThink Code, which has several important implications:
...
- **Execution**: Starts and stops with CoreThink Code (child process lifecycle)
...
- Start when needed by CoreThink Code
- Not require network configuration
- Need to be installed alongside CoreThink Code or via a package manager
...
## Configuring Transports in CoreThink Code

For detailed information on configuring STDIO and SSE transports in CoreThink Code, including example configurations, see the [Understanding Transport Types](/features/mcp/using-mcp-in-kilo-code#understanding-transport-types) section in the Using MCP in CoreThink Code guide.
