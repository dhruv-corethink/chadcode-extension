...
CoreThink Code uses system notifications to inform you about:
...
CoreThink Code's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. CoreThink Code uses macOS's built-in command to display notifications.
...
**How it works:** CoreThink Code first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
