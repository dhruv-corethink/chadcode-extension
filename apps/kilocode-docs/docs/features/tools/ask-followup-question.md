...
This tool creates a conversational interface between CoreThink Code and the user, allowing for gathering clarification, additional details, or user preferences when facing ambiguities or decision points. Each question can include suggested responses to streamline the interaction.

## When is it used?

- When critical information is missing from the original request
- When CoreThink Code needs to choose between multiple valid implementation approaches
- When technical details or preferences are required to proceed
- When CoreThink Code encounters ambiguities that need resolution
...
1. **Information Gap Recognition**: CoreThink Code identifies missing information needed to proceed
2. **Specific Question Creation**: CoreThink Code formulates a clear, targeted question
3. **Suggestion Development**: CoreThink Code creates relevant suggested answers (optional but recommended)
...
10. **Task Continuation**: CoreThink Code proceeds with the task using the new information

## Examples When Used

- When developing a web application, CoreThink Code might ask about preferred styling frameworks (Bootstrap, Tailwind, custom CSS)
- When creating an API, CoreThink Code might ask about authentication methods (JWT, OAuth, API keys)
- When refactoring code, CoreThink Code might ask about prioritizing performance vs. readability
- When setting up a database, CoreThink Code might ask about specific schema design preferences
- When creating a custom feature, CoreThink Code might ask about specific behavior expectations
- When troubleshooting errors, CoreThink Code might ask about specific environment details
