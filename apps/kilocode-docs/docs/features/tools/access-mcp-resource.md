...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows CoreThink Code to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When CoreThink Code needs additional context from external systems
- When CoreThink Code needs to access domain-specific data from specialized MCP servers
- When CoreThink Code needs to retrieve reference documentation hosted by MCP servers
- When CoreThink Code needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
    ...
    - Returns the processed resource data to CoreThink Code for use in the current task
...
- When helping with API development, CoreThink Code retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, CoreThink Code accesses current data samples from connected MCP servers.
- When working in specialized domains, CoreThink Code retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, CoreThink Code references compliance requirements from documentation resources.
