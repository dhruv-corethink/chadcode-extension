...
CoreThink Code implements [Agent Skills](https://agentskills.io/), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
## How Skills Work in CoreThink Code
...
1. **Discovery**: Skills are scanned from designated directories when CoreThink Code initializes
...
In CoreThink Code, the `name` field **must match** the parent directory name:
...
When multiple skills share the same name, CoreThink Code uses these priority rules:
...
Skills are discovered when CoreThink Code initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the CoreThink Code extension.
...
1. **Check the Output panel**: Open `View` → `Output` → Select "CoreThink Code" from dropdown. Look for skill-related errors.
