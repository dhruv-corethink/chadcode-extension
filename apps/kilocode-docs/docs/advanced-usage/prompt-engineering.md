...
Prompt engineering is the art of crafting effective instructions for AI models like CoreThink Code. Well-written prompts lead to better results, fewer errors, and a more efficient workflow.

## General Principles

- **Be Clear and Specific:** Clearly state what you want CoreThink Code to do. Avoid ambiguity.
...
It's often helpful to guide CoreThink Code through a "think-then-do" process:

1.  **Analyze:** Ask CoreThink Code to analyze the current code, identify problems, or plan the approach.
2.  **Plan:** Have CoreThink Code outline the steps it will take to complete the task.
3.  **Execute:** Instruct CoreThink Code to implement the plan, one step at a time.
4.  **Review:** Carefully review the results of each step before proceeding.

## Using Custom Instructions

You can provide custom instructions to further tailor CoreThink Code's behavior. There are two types of custom instructions:
...
- Adjust CoreThink Code's tone or personality.

See the [Custom Instructions](/advanced-usage/custom-instructions) section for more details.

## Handling Ambiguity

If your request is ambiguous or lacks sufficient detail, CoreThink Code might:
...
If CoreThink Code doesn't produce the desired results, you can provide feedback by:

- **Rejecting Actions:** Click the "Reject" button when CoreThink Code proposes an action you don't want.
- **Providing Explanations:** When rejecting, explain _why_ you're rejecting the action. This helps CoreThink Code learn from its mistakes.
...
By following these tips, you can write effective prompts that get the most out of CoreThink Code's capabilities.
