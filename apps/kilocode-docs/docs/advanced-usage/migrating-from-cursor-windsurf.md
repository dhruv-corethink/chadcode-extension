...
Quickly migrate your custom rules from Cursor or Windsurf to CoreThink Code. The process typically takes just a few minutes per project.

**Last Updated**: November 2025

## Why CoreThink Code's Rules System?

CoreThink Code simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in CoreThink Code

| Cursor                                      | CoreThink Code                                 | Key Difference                              |
| ------------------------------------------- | ----------------------------------------- | ------------------------------------------- |
...
**2. Create CoreThink Code directory:**
...
Cursor's `globs` field specifies which files a rule applies to. CoreThink Code handles this through **mode-specific directories** instead.
...
**CoreThink Code approach (Option 1 - Mode-specific directory):**
...
**CoreThink Code approach (Option 2 - Custom mode):**
...
Cursor supports nested `.cursor/rules/` directories. CoreThink Code uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# CoreThink Code: .kilocode/rules/backend-server-api-rules.md
```
...
| Windsurf                                                       | CoreThink Code                      | Key Difference                              |
| -------------------------------------------------------------- | ------------------------------ | ------------------------------------------- |
...
Windsurf configures activation through the GUI. In CoreThink Code, file organization replaces GUI configuration:

| Windsurf GUI Mode        | CoreThink Code Equivalent                                        |
| ------------------------ | ----------------------------------------------------------- |
...
All three tools support the `AGENTS.md` standard. If you have one, it works in CoreThink Code automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - CoreThink Code loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). CoreThink Code also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in CoreThink Code and require user approval to modify.

## Understanding Mode-Specific Rules

This is CoreThink Code's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in CoreThink Code panel
- [ ] **Test rule application:** Ask CoreThink Code to perform tasks following your rules
...
- **After migrating:** Organize files accordingly in CoreThink Code

### Nested Rules Flattened

Cursor's nested directories don't map to CoreThink Code. Flatten with descriptive names:
...
- **Check setting:** Verify "Use Agent Rules" is enabled in CoreThink Code settings (enabled by default)
