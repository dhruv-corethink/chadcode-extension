...
Memory Bank is a system of structured documentation that enables CoreThink Code to **better understand your project** and **maintain context across coding sessions**. It transforms your AI assistant from a stateless tool into a persistent development partner with perfect recall of your project details. CoreThink Code automatically reads your Memory Bank files to rebuild its understanding of your project whenever you start a new session.

When Memory Bank is active, CoreThink Code begins each task with `[Memory Bank: Active]` and a brief summary of your project context, ensuring consistent understanding without repetitive explanations.

## Key Benefits

- **Language Agnostic**: Functions with any programming language or framework
- **Efficient Project Understanding**: Helps CoreThink Code understand the purpose and tech stack of a project
- **Context Preservation**: Maintain project knowledge across sessions without needing to scan files in every new session
- **Faster Startup**: CoreThink Code immediately comprehends your project context when you begin a new session
- **Self-Documenting Projects**: Create valuable documentation as a byproduct

## How Memory Bank Works

Memory Bank is built on CoreThink Code's [Custom Rules](/advanced-usage/custom-rules) feature, providing a specialized framework for project documentation. Memory Bank files are standard markdown files stored in `.kilocode/rules/memory-bank` folder within your project repository. They're not hidden or proprietary - they're regular documentation files that both you and CoreThink Code can access.

At the start of every task, CoreThink Code reads all Memory Bank files to build a comprehensive understanding of your project. This happens automatically without requiring any action from you. CoreThink Code then indicates successful Memory Bank activation with `[Memory Bank: Active]` at the beginning of its response, followed by a brief summary of its understanding of your project.
...
Note: CoreThink Code will not edit this file directly but may suggest improvements if it identifies ways to enhance your project brief.
...
6. Ask CoreThink Code to "initialize memory bank"
7. Wait for CoreThink Code to analyze your project and initialize the Memory Bank files
...
- CoreThink Code will help fill in gaps and ask questions
...
The initialization step is critically important as it establishes the foundation for all future interactions with your project. When you request initialization with the command `initialize memory bank`, CoreThink Code will:
...
:::warning Important
Take time to carefully review and correct the generated files after initialization. Any misunderstandings or missing information at this stage will affect all future interactions. A thorough initialization dramatically improves CoreThink Code's effectiveness, while a rushed or incomplete initialization will permanently limit its ability to assist you effectively.
:::
...
1. CoreThink Code discovers new project patterns
2. After implementing significant changes
3. When you explicitly request with `update memory bank`
4. When context needs clarification

To execute a Memory Bank update, CoreThink Code will:
...
You can direct CoreThink Code to focus on specific information sources by using commands like `update memory bank using information from @/Makefile`.

#### Regular Task Execution

At the beginning of every task, CoreThink Code:
...
At the end of a task, CoreThink Code may suggest updating the memory bank if significant changes were made, using the phrase: "Would you like me to update memory bank to reflect these changes?"
...
To document a task, use the command `add task` or `store this as a task`. CoreThink Code will:
...
When starting a new task, CoreThink Code will check if it matches any documented tasks and follow the established workflow to ensure no steps are missed.
...
CoreThink Code uses status indicators to clearly communicate Memory Bank status:
...
These indicators appear at the beginning of CoreThink Code's responses, providing immediate confirmation of Memory Bank status.
...
As you work with CoreThink Code, your context window will eventually fill up. When you notice responses slowing down or references becoming less accurate:

1. Ask CoreThink Code to "update memory bank" to document the current state
2. Start a new conversation/task
3. CoreThink Code will automatically access your Memory Bank in the new conversation

This process ensures continuity across multiple sessions without losing important context.

## Handling Inconsistencies

If CoreThink Code detects inconsistencies between memory bank files:
...
This ensures that even with imperfect documentation, CoreThink Code can still function effectively.
...
Yes! While CoreThink Code manages most of the files, you can edit any of them manually. The `brief.md` file is specifically designed to be maintained by you. Manual edits to other files will be respected by CoreThink Code.

### What happens if memory bank files are missing?

If memory bank files are missing, CoreThink Code will indicate this with `[Memory Bank: Missing]` at the beginning of its response and suggest initializing the memory bank.
...
Yes! Each project has its own Memory Bank in its `.kilocode/rules/memory-bank/` folder. CoreThink Code will automatically use the correct Memory Bank for each project.
...
- Let CoreThink Code help create the initial structure
...
The Memory Bank is CoreThink Code's only link to previous work. Its effectiveness depends entirely on maintaining clear, accurate documentation and confirming context preservation in every interaction. When you see `[Memory Bank: Active]` at the beginning of a response, you can be confident that CoreThink Code has a comprehensive understanding of your project.
