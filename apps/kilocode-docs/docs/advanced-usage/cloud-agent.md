...
Cloud Agents let you run CoreThink Code in the cloud from any device, without relying on your local machine. They provide a remote development environment that can read and modify your GitHub repositories, run commands, and auto-commit changes as work progresses.

---

## What Cloud Agents Enable

- Run CoreThink Code remotely from a browser
...
- **CoreThink Code credits are still used** when the agent performs work (model usage, operations, etc.).
...
4. **Start chatting with CoreThink Code.**
...
- **Context is persistent across messages.**
  CoreThink Code remembers previous turns within the same session.
