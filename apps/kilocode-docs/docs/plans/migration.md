...
2. **Install CoreThink Code extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on CoreThink Code features and workflows
...
2. **Install CoreThink Code** extension for team members
3. **Start with simple tasks** in CoreThink Code
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use CoreThink Code** for new projects
...
- [ ] Install CoreThink Code extension across team
...
CoreThink Code uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.
...
- **AGENTS.md**: Works identically in CoreThink Code (no conversion needed)
- **Global rules**: Recreate in `~/.kilocode/rules/*.md` directory

CoreThink Code also supports mode-specific rules (`.kilocode/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- CoreThink Code extension installation and setup
