...
# SPEC: CoreThink Code Onboarding & User Engagement Improvements
...
Users aren't guided through CoreThink Code's modes or key features. The existing tab-based tutorial is easily dismissed, causing users to miss critical functionality.
...
The "Set API Key" button is at the bottom of settings, making CoreThink Code setup hard to discover and complete.

**Improvements:**

- Move "Set API Key" button next to API key input field
- Rearrange layout for better flow
- Make CoreThink Code provider setup prominent
...
- Integration with CoreThink Code teams for company/repo-personalized onboarding
