...
The CoreThink Code CLI uses the same underlying technology that powers the IDE extensions, so you can expect the same workflow to handle agentic coding tasks from start to finish.
...
## What you can do with CoreThink Code CLI
...
CoreThink Code automatically creates checkpoints as you work, allowing you to revert to previous states in your project's history.
...
Parallel mode allows multiple CoreThink Code instances to work in parallel on the same directory, without conflicts. You can spawn as many CoreThink Code instances as you need! Once finished, changes will be available on a separate git branch.
...
Auto-approval allows the CoreThink Code CLI to perform operations without first requiring user confirmation. These settings can either be built up over time in interactive mode, or by editing your config file using `kilocode config` or editing the file directly at `~/.kilocode/config.json`.
...
Interactive mode is the default mode when running CoreThink Code without the `--auto` flag, designed to work interactively with a user through the console.

In interactive mode CoreThink Code will request approval for operations which have not been auto-approved, allowing the user to review and approve operations before they are executed, and optionally add them to the auto-approval list.
...
Autonomous mode allows CoreThink Code to run in automated environments like CI/CD pipelines without requiring user interaction.
...
Use the `--json` flag with `--auto` to get structured JSON output instead of the default terminal UI. This is useful for programmatic integration and parsing of CoreThink Code responses.
