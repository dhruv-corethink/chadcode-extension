---
sidebar_label: CoreThink Code Provider
---

# Using CoreThink Code's Built-in Provider

CoreThink Code provides its own built-in API provider that gives you access to the latest frontier coding models through a simple registration process. No need to manage API keys from multiple providers - just sign up and start coding.

**Website:** [https://kilo.ai/](https://kilo.ai/)

## Getting Started

When you sign up for CoreThink Code, you can start immediately with free models, or top up your account for the first time to get bonus credits.
...
CoreThink Code offers a streamlined registration that connects you directly to frontier coding models:

1. **Start Registration:** Click "Try CoreThink Code for Free" in the extension
...
<!-- <img src="/img/setting-up/signupflow.gif" alt="Sign up and registration flow with CoreThink Code" width="600" /> -->

## Supported Models

CoreThink Code provides access to the latest frontier coding models through its built-in provider. The specific models available are automatically updated and managed by the CoreThink Code service, ensuring you always have access to the most capable models for coding tasks.

## Configuration in CoreThink Code

Once you've completed the registration process, CoreThink Code is automatically configured:

1. **Automatic Setup:** After successful registration, CoreThink Code is ready to use immediately
2. **No API Key Management:** Your authentication is handled seamlessly through the registration process
3. **Model Selection:** Access to frontier models is provided automatically through your CoreThink Code account

### Provider Routing

CoreThink Code can route to many different inference providers. For personal accounts, provider routing behavior can be controlled in the API Provider settings under Provider Routing.
...
With the CoreThink Code provider, if you sign up with Google you can also connect other sign in accounts - like GitHub - by:
...
- **Free Credits:** New users receive free credits to explore CoreThink Code's capabilities
...
For detailed setup instructions, see [Setting up CoreThink Code](/getting-started/setting-up).
