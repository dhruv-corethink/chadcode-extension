...
# Using OpenAI Compatible Providers With CoreThink Code

CoreThink Code supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This means you can use models from providers _other than_ OpenAI, while still using a familiar API interface. This includes providers like:
...
You'll find these settings in the CoreThink Code settings panel (click the <Codicon name="gear" /> icon):
...
CoreThink Code supports full endpoint URLs in the Base URL field, providing greater flexibility for provider configuration:
...
While this provider type allows connecting to various endpoints, if you are connecting directly to the official OpenAI API (or an endpoint mirroring it exactly), CoreThink Code recognizes the following model IDs based on the `openAiNativeModels` definition in its source code:
...
By using an OpenAI-compatible provider, you can leverage the flexibility of CoreThink Code with a wider range of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
