...
CoreThink Code 的系统通知可在所有主要操作系统上运行，但底层技术不同：
...
无需额外设置。CoreThink Code 使用 macOS 的内置命令来显示通知。
...
**工作原理：** CoreThink Code 会首先尝试使用 `terminal-notifier`，如果未安装则自动回退到 AppleScript。
