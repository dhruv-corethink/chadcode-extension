...
模型上下文协议（MCP）支持两种主要的传输机制用于CoreThink Code与MCP服务器之间的通信：标准输入/输出（STDIO）和服务器发送事件（SSE）。每种机制都有其独特的特点、优势和适用场景。
...
1. 客户端（CoreThink Code）将MCP服务器作为子进程启动
...
- 安全敏感操作
- 低延迟要求
- 单客户端场景（每个服务器一个CoreThink Code实例）
- 命令行工具或IDE扩展
...
1. 客户端（CoreThink Code）通过HTTP GET请求连接到服务器的SSE端点
...
- **远程访问**：可以托管在与CoreThink Code不同的机器上
- **可扩展性**：支持多个客户端并发连接
- **协议**：基于标准HTTP（无需特殊协议）
...
STDIO服务器与CoreThink Code在同一台机器上运行，这有几个重要影响：
...
- **执行**：随CoreThink Code启动和停止（子进程生命周期）
- **依赖**：任何依赖项都必须安装在用户机器上
...
- 在用户机器上运行
- 直接访问本地文件系统
- 在CoreThink Code需要时启动
- 不需要网络配置
- 需要与CoreThink Code一起安装或通过包管理器安装
...
## 在CoreThink Code中配置传输

有关在CoreThink Code中配置STDIO和SSE传输的详细信息，包括示例配置，请参见[理解传输类型](/features/mcp/using-mcp-in-kilo-code#understanding-transport-types)部分。
