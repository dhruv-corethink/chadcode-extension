...
`switch_mode` 工具使 CoreThink Code 能够在不同操作模式之间切换，每种模式都具有针对特定任务类型的专业能力。当当前任务需要不同的专业知识时，可以无缝切换到 Code、Architect、Ask 或 Debug 模式。
...
该工具在当当前任务更适合由其他模式处理时请求模式切换。它在切换过程中保持上下文，同时调整 CoreThink Code 的焦点和可用工具集，以匹配新任务阶段的需求。
...
- 讨论新功能时，CoreThink Code 从 Ask 模式切换到 Architect 模式以帮助设计系统结构。
- 在 Architect 模式中完成架构规划后，CoreThink Code 切换到 Code 模式以实现设计的功能。
- 在开发过程中遇到错误时，CoreThink Code 从 Code 模式切换到 Debug 模式进行系统问题排查。
