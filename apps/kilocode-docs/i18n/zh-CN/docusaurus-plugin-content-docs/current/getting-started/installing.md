---
sidebar_label: 安装CoreThink Code
---

# 安装CoreThink Code

CoreThink Code是一款VS Code扩展，可将AI编程助手直接集成到编辑器中。可通过以下方式安装：
...
若已安装VS Code：[点击此处直接安装CoreThink Code](vscode:extension/kilocode.Kilo-Code)
...
3. 搜索 "CoreThink Code"
4. 选择CoreThink Code发布的扩展并点击**安装**
5. 根据提示重新加载VS Code

安装完成后，在侧边栏找到CoreThink Code图标（<img src="/docs/img/kilo-v1.svg" width="12" />）即可打开面板。

<img src="/docs/img/installing/installing.png" alt="VS Code应用商店中的CoreThink Code扩展安装界面" width="400" />
*VS Code应用商店中的CoreThink Code扩展安装界面*

## Cursor应用商店

:::tip

若已安装Cursor：[点击此处直接安装CoreThink Code](cursor:extension/kilocode.Kilo-Code)
...
在VSCodium、Gitpod、Eclipse Theia、Windsurf等兼容编辑器中，可通过[Open VSX上的CoreThink Code页面](https://open-vsx.org/extension/kilocode/Kilo-Code)直接安装。
...
1. **下载VSIX文件：**
    ...
    - 在[CoreThink Code GitHub Releases页面](https://github.com/Kilo-Org/kilocode/releases)查找正式版本
...
- 重启VS Code
- 检查扩展列表确认CoreThink Code已启用
- 尝试禁用后重新启用扩展
- 在输出面板查看错误日志（查看 → 输出，选择"CoreThink Code"）
