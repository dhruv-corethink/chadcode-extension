...
# 在CoreThink Code中使用AWS Bedrock

CoreThink Code支持通过Amazon Bedrock访问模型，这是一个完全托管的服务，通过单一API提供来自领先AI公司的高性能基础模型(FMs)。
...
根据源代码，CoreThink Code支持通过Bedrock访问以下模型：
...
请参考[Amazon Bedrock文档](https://docs.aws.amazon.com/bedrock/latest/userguide/models-supported.html)获取最新可用模型及其ID列表。配置CoreThink Code时请使用*模型ID*而非模型名称。

## 在CoreThink Code中配置

1.  **打开CoreThink Code设置:** 点击CoreThink Code面板中的齿轮图标(<Codicon name="gear" />)
