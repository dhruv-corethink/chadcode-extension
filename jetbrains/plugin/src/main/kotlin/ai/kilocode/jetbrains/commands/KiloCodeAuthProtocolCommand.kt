...
/**
 * JetBrains Protocol Command for handling CoreThink Code authentication URLs
 *
...
class KiloCodeAuthProtocolCommand : JBProtocolCommand("ai.kilocode.jetbrains.auth") {
    ...
    override suspend fun execute(target: String?, parameters: Map<String, String>, fragment: String?): String? {
        logger.info("Handling CoreThink Code auth protocol command: target=$target, parameters=$parameters")
        ...
        return try {
            ...
            val errorMsg = "Error handling CoreThink Code auth protocol command: ${e.message}"
